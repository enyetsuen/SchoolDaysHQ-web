...
function _JS_Video_CanPlayFormat(format) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(80, 1, format);
...
function _JS_Video_Create(url) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(81, 1, url);
...
function _JS_Video_Destroy(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(82, 1, video);
...
function _JS_Video_Duration(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(83, 1, video);
 return videoInstances[video].duration;
}

function _JS_Video_EnableAudioTrack(video, trackIndex, enabled) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(84, 1, video, trackIndex, enabled);
 var v = videoInstances[video];
...
function _JS_Video_GetAudioLanguageCode(video, trackIndex, buffer, bufferSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(85, 1, video, trackIndex, buffer, bufferSize);
...
function _JS_Video_GetPlaybackRate(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(87, 1, video);
...
function _JS_Video_SetLoop(video, loop) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(94, 1, video, loop);
 var v = videoInstances[video];
 if (v.loopEndPollInterval) {
  clearInterval(v.loopEndPollInterval);
 }
...
function _JS_Video_Play(video, muted) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(93, 1, video, muted);
 var v = videoInstances[video];
...
function _JS_Video_Seek(video, time) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(95, 1, video, time);
 var v = videoInstances[video];
 v.lastSeenPlaybackTime = v.currentTime = time;
}

function _JS_Video_SetEndedHandler(video, ref, onended) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(96, 1, video, ref, onended);
 var v = videoInstances[video];
...
function _JS_Video_SetErrorHandler(video, ref, onerror) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(97, 1, video, ref, onerror);
...
function _JS_Video_SetMute(video, muted) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(98, 1, video, muted);
 var v = videoInstances[video];
 v.muted = muted || jsVideoAllAudioTracksAreDisabled(v);
}

function _JS_Video_SetPlaybackRate(video, rate) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(99, 1, video, rate);
 videoInstances[video].playbackRate = rate;
}

function _JS_Video_SetReadyHandler(video, ref, onready) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(100, 1, video, ref, onready);
...
function _JS_Video_SetSeekedHandler(video, ref, onseeked) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(101, 1, video, ref, onseeked);
...
function _JS_Video_SetVolume(video, volume) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(102, 1, video, volume);
 videoInstances[video].volume = volume;
}

function _JS_Video_Time(video) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(103, 1, video);
...
function _JS_Video_UpdateToTexture(video, tex, adjustToLinearspace) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(104, 1, video, tex, adjustToLinearspace);
 var v = videoInstances[video];
...
function _JS_WebCamVideo_GetNativeHeight(deviceId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(106, 1, deviceId);
 return activeWebCams[deviceId] && activeWebCams[deviceId].video.videoHeight;
}

function _JS_WebCamVideo_GetNativeWidth(deviceId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(107, 1, deviceId);
...
function _JS_WebCamVideo_GrabFrame(deviceId, buffer, destWidth, destHeight) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(108, 1, deviceId, buffer, destWidth, destHeight);
...
function _JS_WebPlayer_FinishInitialization() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(109, 1);
...
function _JS_WebRequest_Abort(requestId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(110, 1, requestId);
...
function _JS_WebRequest_Create(url, method) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(111, 1, url, method);
...
function _JS_WebRequest_GetResponseMetaData(requestId, headerBuffer, headerSize, responseUrlBuffer, responseUrlSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(112, 1, requestId, headerBuffer, headerSize, responseUrlBuffer, responseUrlSize);
 var response = wr.responses[requestId];
 if (!response) {
...
function _JS_WebRequest_GetResponseMetaDataLengths(requestId, buffer) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(113, 1, requestId, buffer);
 buffer = buffer >> 2;
 var response = wr.responses[requestId];
 if (!response) {
...
function _JS_WebRequest_Release(requestId) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(114, 1, requestId);
...
function _JS_WebRequest_Send(requestId, ptr, length, arg, onresponse, onprogress) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(115, 1, requestId, ptr, length, arg, onresponse, onprogress);
 ptr = ptr;
...
function _JS_WebRequest_SetRedirectLimit(request, redirectLimit) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(116, 1, request, redirectLimit);
...
function _JS_WebRequest_SetRequestHeader(requestId, header, value) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(117, 1, requestId, header, value);
 var requestOptions = wr.requests[requestId];
 if (!requestOptions) {
  return;
 }
...
function _JS_WebRequest_SetTimeout(requestId, timeout) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(118, 1, requestId, timeout);
 var requestOptions = wr.requests[requestId];
 if (!requestOptions) {
  return;
 }
...
function pthreadCreateProxied(pthread_ptr, attr, startRoutine, arg) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(119, 1, pthread_ptr, attr, startRoutine, arg);
...
function ___syscall__newselect(nfds, readfds, writefds, exceptfds, timeout) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(120, 1, nfds, readfds, writefds, exceptfds, timeout);
 try {
...
function ___syscall_accept4(fd, addr, addrlen, flags, d1, d2) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(121, 1, fd, addr, addrlen, flags, d1, d2);
 try {
...
function ___syscall_chmod(path, mode) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(122, 1, path, mode);
 try {
...
function ___syscall_connect(fd, addr, addrlen, d1, d2, d3) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(123, 1, fd, addr, addrlen, d1, d2, d3);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_faccessat(dirfd, path, amode, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(124, 1, dirfd, path, amode, flags);
 try {
  path = SYSCALLS.getStr(path);
  assert(flags === 0);
...
function ___syscall_fcntl64(fd, cmd, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(125, 1, fd, cmd, varargs);
...
function ___syscall_fstat64(fd, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(126, 1, fd, buf);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function ___syscall_getcwd(buf, size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(127, 1, buf, size);
 try {
  if (size === 0) return -28;
  var cwd = FS.cwd();
...
function ___syscall_getdents64(fd, dirp, count) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(128, 1, fd, dirp, count);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  if (!stream.getdents) {
...
function ___syscall_getsockopt(fd, level, optname, optval, optlen, d1) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(129, 1, fd, level, optname, optval, optlen, d1);
 try {
  var sock = getSocketFromFD(fd);
  if (level === 1) {
   if (optname === 4) {
...
function ___syscall_ioctl(fd, op, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(130, 1, fd, op, varargs);
 SYSCALLS.varargs = varargs;
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  switch (op) {
...
function ___syscall_lstat64(path, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(131, 1, path, buf);
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_mkdirat(dirfd, path, mode) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(132, 1, dirfd, path, mode);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
  path = PATH.normalize(path);
...
function ___syscall_newfstatat(dirfd, path, buf, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(133, 1, dirfd, path, buf, flags);
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_openat(dirfd, path, flags, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(134, 1, dirfd, path, flags, varargs);
 SYSCALLS.varargs = varargs;
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
...
function ___syscall_readlinkat(dirfd, path, buf, bufsize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(135, 1, dirfd, path, buf, bufsize);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
...
function ___syscall_recvfrom(fd, buf, len, flags, addr, addrlen) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(136, 1, fd, buf, len, flags, addr, addrlen);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_renameat(olddirfd, oldpath, newdirfd, newpath) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(137, 1, olddirfd, oldpath, newdirfd, newpath);
 try {
...
function ___syscall_rmdir(path) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(138, 1, path);
 try {
  path = SYSCALLS.getStr(path);
  FS.rmdir(path);
  return 0;
...
function ___syscall_sendto(fd, message, length, flags, addr, addr_len) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(139, 1, fd, message, length, flags, addr, addr_len);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_socket(domain, type, protocol) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(140, 1, domain, type, protocol);
 try {
...
function ___syscall_stat64(path, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(141, 1, path, buf);
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_statfs64(path, size, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(142, 1, path, size, buf);
 try {
  path = SYSCALLS.getStr(path);
  assert(size === 64);
...
function ___syscall_symlink(target, linkpath) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(143, 1, target, linkpath);
 try {
...
function ___syscall_truncate64(path, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(144, 1, path, length);
 try {
...
function ___syscall_unlinkat(dirfd, path, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(145, 1, dirfd, path, flags);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
  if (flags === 0) {
...
function ___syscall_utimensat(dirfd, path, times, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(146, 1, dirfd, path, times, flags);
 try {
  path = SYSCALLS.getStr(path);
  assert(flags === 0);
...
function __mmap_js(len, prot, flags, fd, off, allocated, addr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(147, 1, len, prot, flags, fd, off, allocated, addr);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function __munmap_js(addr, len, prot, flags, fd, offset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(148, 1, addr, len, prot, flags, fd, offset);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  if (prot & 2) {
...
function getCanvasSizeMainThread(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(150, 1, target, width, height);
...
function setCanvasElementSizeMainThread(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(151, 1, target, width, height);
...
function _emscripten_get_fullscreen_status(fullscreenStatus) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(153, 1, fullscreenStatus);
 if (!JSEvents.fullscreenEnabled()) return -1;
...
function _emscripten_get_gamepad_status(index, gamepadState) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(154, 1, index, gamepadState);
...
function _emscripten_request_fullscreen(target, deferUntilInEventHandler) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(156, 1, target, deferUntilInEventHandler);
 var strategy = {
  scaleMode: 0,
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(157, 1, target, deferUntilInEventHandler);
 target = findEventTarget(target);
 if (!target) return -4;
 if (!target.requestPointerLock) {
  return -1;
...
function _emscripten_set_blur_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(159, 1, target, userData, useCapture, callbackfunc, targetThread);
 return registerFocusEventCallback(target, userData, useCapture, callbackfunc, 12, "blur", targetThread);
}

function _emscripten_set_focus_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(160, 1, target, userData, useCapture, callbackfunc, targetThread);
...
function _emscripten_set_fullscreenchange_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(161, 1, target, userData, useCapture, callbackfunc, targetThread);
 if (!JSEvents.fullscreenEnabled()) return -1;
 target = findEventTarget(target);
 if (!target) return -4;
...
function _emscripten_set_gamepadconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(162, 1, userData, useCapture, callbackfunc, targetThread);
 if (_emscripten_sample_gamepad_data()) return -1;
 return registerGamepadEventCallback(2, userData, useCapture, callbackfunc, 26, "gamepadconnected", targetThread);
}

function _emscripten_set_gamepaddisconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(163, 1, userData, useCapture, callbackfunc, targetThread);
 if (_emscripten_sample_gamepad_data()) return -1;
...
function _emscripten_set_keypress_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(165, 1, target, userData, useCapture, callbackfunc, targetThread);
...
function _environ_get(__environ, environ_buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(176, 1, __environ, environ_buf);
 var bufSize = 0;
...
function _environ_sizes_get(penviron_count, penviron_buf_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(177, 1, penviron_count, penviron_buf_size);
...
function _fd_close(fd) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(178, 1, fd);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  FS.close(stream);
  return 0;
...
function _fd_fdstat_get(fd, pbuf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(179, 1, fd, pbuf);
 try {
  var rightsBase = 0;
...
function _fd_read(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(180, 1, fd, iov, iovcnt, pnum);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_seek(fd, offset, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(181, 1, fd, offset, whence, newOffset);
 try {
...
function _fd_write(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(182, 1, fd, iov, iovcnt, pnum);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
